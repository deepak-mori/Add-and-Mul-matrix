...
__global__ void per_row_AB_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    for(long int i=0; i<n; i++){
        for(long int j=0; j<n; j++){
            C[(id/m)*m*n*n + i*m + j*m*n + (id%m)] = A[(id/m)*n + i] * B[(id%m)*n + j];
        }
    }
...
__global__ void per_column_AB_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    long int id2 = threadIdx.x * blockDim.y + threadIdx.y;

    if(id2 < n){
        for(long int i=0; i<m; i++){
            for(long int j=0; j<m; j++){
                long int C_ind = id1*m + i*m*n*n + j + id2*m*n;
                C[C_ind] = A[id1 + i*n] * B[id2 + j*n];
            }
        }
    }
...
__global__ void per_element_kernel(long int *A, long int *B, long int *C,long int m, long int n){
    ...
    // --> Complete the kernel ....
    long int id = blockDim.y * threadIdx.x + threadIdx.y + (blockIdx.x * gridDim.y + blockIdx.y)*(blockDim.x * blockDim.y * blockDim.z);
    ...
    if(id < m*n*m*n){
        ...
        C[id] = A[rowa * n + cola] * B[rowb * n + colb];
...
int main(int argc,char **argv){
    ...
    // --> Set the launch configuration 
    gridDimx = n;
    gridDimy = ceil((float)n/2);
    dim3 block(gridDimy, 2, 1);

    starttime = rtclock(); 

    // --> Launch the kernel 
    per_column_AB_kernel<<<gridDimx , block>>>(d_a , d_b , d_c , m , n);
    cudaDeviceSynchronize(); 
